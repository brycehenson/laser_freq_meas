...
laser_freq=[];
laser_freq.mean=wmean(spectrum.freqs,db2pow(spectrum.pow));
laser_freq.std=std(spectrum.freqs,db2pow(spectrum.pow));

%%
xscale=1e-6;
predictor=spectrum.freqs*xscale;
...
beta0=[laser_freq.std*xscale,laser_freq.mean*xscale,max(response),0];
...
fitobj=fitnlm(predictor,response,fit_fun,beta0,...
    'options',opt,...
    'CoefficientNames',cof_names,'ErrorModel','proportional');
